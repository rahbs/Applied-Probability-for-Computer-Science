import pandas as pd
from functions import gaussian_bayes_classifier, calculate_mean, calculate_std, calculate_precision, calculate_recall, calculate_avg
...
for i in range(k):
    ...
    for class_ in range (n_class):
        ...
        for col in range (n_cols-1):
            x = train_data[train_data['class'] == class_].iloc[:,col].tolist()
            mean = calculate_mean(x)
            std = calculate_std(x,mean)
            mean_list_per_class.append(mean)
            std_list_per_class.append(std)
        mean_list.append(mean_list_per_class)
        std_list.append(std_list_per_class)
    ...
    for idx, row in val_data.iterrows():
        ...
        # gaussian classifer
        val_class_hat = gaussian_bayes_classifier(val_x, mean_list, std_list)
